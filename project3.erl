...
-module(project3).
-export([lastNodetoCurrent/2,  notify/2, updateFingerTables/1, create/2, 
  findNextNode/3, joinNodetoChord/2, stabilizeChord/1,runApp/2, reverse/1, updateAtIdx/3]).
...
runApp(NumofNodes, NumofRequests) ->
  ...
  Nodes = [{ N - 1, spawn_link(project3, create, [LogNodes, N - 1]) } || N <- lists:seq(1, NumofNodes)],
  startApp(Nodes, NumofRequests).
